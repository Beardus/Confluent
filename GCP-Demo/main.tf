...
# Define a Confluent environment named "GCPDemo" with an "Advanced" Stream Governance package.
resource "confluent_environment" "GCPDemo" {
  display_name = "GCP_Demo"
...
# Retrieve information about an existing Schema Registry cluster for use in the "GCPDemo" environment.
...
data "confluent_schema_registry_cluster" "advanced" {
  environment {
    id = confluent_environment.GCPDemo.id
  }
...
# This cluster will be single-zone, hosted on GCP in the "us-central1" region, and part of the "GCPDemo" environment.
...
# in the 'GCPDemo' environment. Depends on the role binding to ensure proper permissions.
...
resource "confluent_role_binding" "env-manager-kafka-environment-admin" {
  ...
  crn_pattern = confluent_environment.GCPDemo.resource_name
}
...
resource "confluent_kafka_topic" "users_salesforce" {
  kafka_cluster {
    id = confluent_kafka_cluster.standard.id
  }
  topic_name    = "users_salesforce"
  rest_endpoint = confluent_kafka_cluster.standard.rest_endpoint
  credentials {
    key    = confluent_api_key.apps-manager-kafka-api-key.id
    secret = confluent_api_key.apps-manager-kafka-api-key.secret
...
resource "confluent_kafka_acl" "app-connector-write-on-smt-prefix-topics" {
  kafka_cluster {
    id = confluent_kafka_cluster.standard.id
  }
  resource_type = "TOPIC"
  resource_name = local.smt_server_name
  pattern_type  = "PREFIXED"
  principal     = "User:${confluent_service_account.app-connector.id}"
  host          = "*"
  operation     = "WRITE"
...
resource "confluent_kafka_acl" "app-connector-create-on-smt-prefix-topics" {
  kafka_cluster {
    id = confluent_kafka_cluster.standard.id
  }
  resource_type = "TOPIC"
...
resource "confluent_kafka_acl" "app-connector-write-on-target-topic-users-salesforce" {
  kafka_cluster {
    id = confluent_kafka_cluster.standard.id
  }
  resource_type = "TOPIC"
  resource_name = confluent_kafka_topic.users_salesforce.topic_name
  pattern_type  = "LITERAL"
  principal     = "User:${confluent_service_account.app-connector.id}"
  host          = "*"
  operation     = "WRITE"
...
resource "confluent_kafka_acl" "app-connector-read-on-target-topic-users-salesforce" {
  kafka_cluster {
    id = confluent_kafka_cluster.standard.id
  }
  resource_type = "TOPIC"
...
# This pool runs on GCP in the 'eu-west-1' region with a maximum of 10 CFUs (Compute Flink Units).
# The pool is associated with the specified 'GCPDemo' environment.
...
resource "confluent_connector" "users_gcs_sink" {
  environment {
    id = confluent_environment.GCPDemo.id
  }
  kafka_cluster {
  ...
  config_sensitive = {
    "gcs.credentials.config"   = var.gcs_credentials
  }
...
resource "confluent_connector" "postgre-sql-cdc-source-smt" {
  environment {
    id = confluent_environment.GCPDemo.id
  }
  kafka_cluster {
  ...
  config_sensitive = {
    "database.password" = var.post_password
  }
  ...
  config_nonsensitive = {
    ...
    "name"                     = "PostgresCdcSourceSmt"
    "kafka.auth.mode"          = "SERVICE_ACCOUNT"
    "kafka.service.account.id" = confluent_service_account.app-connector.id
    "database.hostname"        = "35.224.151.166"
    "database.port"            = "5432"
    "database.user"            = "ccpostgresql"
    "database.dbname"          = "Sample"
    "database.server.name"     = local.smt_server_name,
    ...
    "topic.prefix"             = "smt",
    "transforms"               = "unwarp",
    "transforms.unwarp.delete.handling.mode" = "none",
    "transforms.unwarp.drop.tombstones" = "false",
    "transforms.unwarp.type"   = "io.debezium.transforms.ExtractNewRecordState"
  }

  depends_on = [
    confluent_kafka_acl.app-connector-describe-on-cluster,
    confluent_kafka_acl.app-connector-write-on-smt-prefix-topics,
    confluent_kafka_acl.app-connector-create-on-smt-prefix-topics,
    confluent_kafka_acl.app-connector-read-on-target-topic-pageviews,
    confluent_kafka_acl.app-connector-create-on-dlq-lcc-topics,
    confluent_kafka_acl.app-connector-write-on-dlq-lcc-topics,
    confluent_kafka_acl.app-connector-create-on-success-lcc-topics,
...
resource "confluent_connector" "salesforce-cdc-source-smt" {
  environment {
    id = confluent_environment.GCPDemo.id
  }
  kafka_cluster {
  ...
  // https://docs.confluent.io/cloud/current/connectors/cc-postgresql-cdc-source-debezium.html#configuration-properties
  config_sensitive = {
    "salesforce.username"             = var.sales_user
    "salesforce.password"             = var.sales_password
    "salesforce.password.token"       = var.sales_token
    "salesforce.consumer.key"         = var.sales_key
    "salesforce.consumer.secret"      = var.sales_secret
  }
  ...
  config_nonsensitive = {
    ...
    "kafka.service.account.id"        = confluent_service_account.app-connector.id
    "tasks.max"                       = "1"
    "kafka.topic"                     = "users_salesforce"
    "salesforce.cdc.name"             = "UserAccount__ChangeEvent"
    "salesforce.grant.type"           = "PASSWORD" 
    "salesforce.channel.type"         = "SINGLE"
    "output.data.format"              = "JSON_SR"
    //"confluent.license"               = "your_confluent_license"  # Optional, required for enterprise features
  }

  depends_on = [
    confluent_kafka_acl.app-connector-describe-on-cluster,
    confluent_kafka_acl.app-connector-write-on-target-topic-users-salesforce,
    confluent_kafka_acl.app-connector-create-on-dlq-lcc-topics,
    confluent_kafka_acl.app-connector-write-on-dlq-lcc-topics,
    confluent_kafka_acl.app-connector-create-on-success-lcc-topics,
    confluent_kafka_acl.app-connector-write-on-success-lcc-topics,
